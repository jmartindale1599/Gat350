...
namespace neu {
	...
	struct Vector3 {
		...
		//add z to all of it bruuuuuuuuuuuuuh
		...
		Vector3& operator += (const float s) { this->x += s, this->y += s, this->z += s; return *this; }
	...
	inline float Vector3::DistanceSqr(const Vector3 v){
		...
		return ((*this) - v).LegnthSquare();;
	
	}

	inline float Vector3::Distance(const Vector3 v){

		return ((*this) - v).Legnth();;
